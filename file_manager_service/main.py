...
from services.auth_connection_service import AuthConnectionService
from services.database_connection_service import DatabaseConnectionService
from middleware import ExceptionHandlerMiddleware, AuthMiddleware
...
def get_openapi_schema(app: FastAPI):
    ...
    for path_name, path_item in openapi_schema["paths"].items():
        if path_name not in ["/health", "/docs", "/openapi.json"]:
...
class ServiceConfig:
    ...
    async def print_startup_logs(self, mode: str):
        ...
        await auth_service.test_connection()

        rabbitmq_service = RabbitMQConnectionService()
        await rabbitmq_service.test_connection()
...
config = ServiceConfig(
    ...
    service_description="File Upload/Download and Queue Messaging Service"
)
